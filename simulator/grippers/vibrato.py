...
PATH_TO_GRIPPER_MODEL = os.path.expanduser(cwd + "/models/grippers/vibrato")
PATH_TO_GRIPPER_XML = os.path.join(PATH_TO_GRIPPER_MODEL, "vibrato_gripper.xml")
...
class VibratoGripperBase(GripperModel):
    """
...
class VibratoGripper(VibratoGripperBase):
    """
    ...
    def _set_key_map(self):
        ...
        for key in VIBRATO_MAP["joint"].keys():
            self._key_map["joint"].update(
                {key: self.naming_prefix + VIBRATO_MAP["joint"][key]}
            )
            self._key_map["actuator"].update(
                {key: self.naming_prefix + VIBRATO_MAP["actuator"][key]}
            )
